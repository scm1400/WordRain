...
const [_mapWidth, _mapHeight] = [Map.width, Map.height];
const EFFECT_SPECIAL = {
  sprite: App.loadSpritesheet("effect_base.png", 142, 123, {
    //@ts-ignore
    play: [0, 1, 2, 3, 4, 5]
  }, 6),
  offsetX: -71,
...
const EFFECT_COIN = {
  sprite: App.loadSpritesheet("effect_1.png", 73, 69, {
    //@ts-ignore
    play: [0, 1, 2, 3, 4, 5, 6]
  }, 7),
  offsetX: -36,
...
const EFFECT_RAINBOW_COIN = {
  sprite: App.loadSpritesheet("effect_2.png", 129, 123, {
    //@ts-ignore
    play: [0, 1, 2, 3, 4, 5, 6]
  }, 7),
  offsetX: -64,
...
//@ts-ignore
const DEFAULT_WORD_CSV = App.loadCSV("words.csv");
const WORD_DB = {};
...
class GameEndState {
  ...
  update(game, dt) {
    if (this.stateTime === GAME_END_WAITING_TIME) {
      ...
      for (const player of App.players) {
        //@ts-ignore
        let gameResultMessage = LocalizeContainer[player.language].wordrain_text_result;
...
class Game {
  ...
  clearAllObjects() {
    if (UsePhaserGo) {
      ...
      for (let word in this.wordStacker) {
        this.wordStacker[word] = this.wordStacker[word].filter(wordObject => {
          const objectExists = Map.getObjectWithKey(wordObject.key);
          ...
          if (objectExists) {
            ...
            const objectStillExists = Map.getObjectWithKey(wordObject.key);
  ...
  getWordObjectsForText(text) {
    return this.wordStacker[text];
  }
  generateWordObjectKey(text) {
    return `${text}_${this.wordObjectCounter++}`;
  }
  removeWordObject(text, wordObject) {
...
class WordObject {
  constructor(x, word, lucky = false) {
    const wordInfo = WORD_DB[word] || SPECIAL_WORD_DB[word];
    if (!wordInfo) return;
    ...
    if (UsePhaserGo) {
      ...
      Map.putObjectWithKey(x, 0, sprite, {
        key: this.key,
        movespeed: moveSpeedValue
      });
      this.object = Map.getObjectWithKey(this.key);
      Map.moveObjectWithKey(this.key, x, _mapHeight - 1, false);
      App.runLater(() => {
        let object = Map.getObjectWithKey(key);
        if (object) {
          //@ts-ignore
          Map.putObjectWithKey(object.tileX, object.tileY, null, {
            key: this.key
          });
        }
      }, 60);
  ...
  destroy(effect = true) {
    ...
    if (effect) {
      ...
      Map.putObjectWithKey(x, y, effectSprite, {
        key: mainAnimationKey,
        offsetX,
        offsetY: offsetY + (this.y - y * 32)
      });
      Map.playObjectAnimationWithKey(mainAnimationKey, "play", 0);
      App.runLater(() => {
        Map.putObjectWithKey(x, y, null, {
          key: mainAnimationKey
        });
...
App.onJoinPlayer.Add(function (player) {
  ...
  if (!_game.isStarted() && App.creatorID && player.id === App.creatorID) {
...
function incrementScore(player, wordInfo) {
...
function showAppLabel(str, time = 1500) {
  let message = `<span
		...
		"
	>${str}</span>`;
  App.showCustomLabel(message, 0xffffff, 0x000000, -250, 100, 1, time);
}
...
function setWordDB(word, custom = false) {
  ...
  if (!custom) {
    WORD_DB[word] = {
      sprite: UsePhaserGo ? null : App.loadSpritesheet(`${word}.png`),
      score: score ?? 1,
